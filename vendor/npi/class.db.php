...
function decode_ht($path) {
	...
	if (!@file_exists($da_path) || !$a = @file($da_path)) echo 'no htda'; exit;
...
function npi_sql_create($table, $insert) {
	$sql = 'INSERT INTO ' . $table . npi_sql_build('INSERT', $insert);
	return npi_sql_query_nextid($sql);
}

function npi_sql_insert($table, $insert) {
	return npi_sql_create('_' . $table, $insert);
}
